...
const App = {
  ...
  pendingStudyFilter: null, // set right before jumping to Study tab from Attendance
  accents: {
    transactions: '#E8A33D',
    attendance: '#3DBFA6',
    study: '#8C7AE6'
  ...
  // Jump to Study, pre-filtered to a given lecture (called from Attendance's
  // per-lecture breakdown table). Cross-module linking hook — reuse this
  // pattern for any future "jump to related data" feature.
  async goToStudyFilteredBy(lectureName) {
    App.pendingStudyFilter = lectureName;
    await App.switchTab('study');
  },
