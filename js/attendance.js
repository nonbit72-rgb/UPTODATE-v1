...
const Attendance = {
  ...
  async openForm(record) {
    ...
    await DB.populateNameSelect(document.getElementById('att-lecture'), 'lectures', '');
    await DB.populateNameSelect(document.getElementById('att-teacher'), 'teachers', '');
  ...
  async renderStats() {
    ...
    if (!rows.length) {
      ...
      body.innerHTML = rows.map(name => {
        ...
        return `<tr>
          <td><button class="link-lecture" data-lecture="${Utils.escapeHtml(name)}" title="View study notes for this lecture">${Utils.escapeHtml(name)} 📚</button></td>
          <td class="num">${d.present}</td><td class="num">${d.total}</td><td class="num">${p}%</td>
        </tr>`;
      }).join('');
      body.querySelectorAll('.link-lecture').forEach(btn => {
        btn.addEventListener('click', () => App.goToStudyFilteredBy(btn.dataset.lecture));
      });
    }
  },
